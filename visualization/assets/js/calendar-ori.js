...
var colorScale = d3.scale.threshold() //based on http://www.perbang.dk/rgbgradient/ from #eee to #FF8C00
      .range(['#eeeeee','#F2D5B2','#F6BD77','#FAA43B','#ff6c02', '#ff0202', '#fc499c','#d60264','#910143','#cb6fe8']);
...
svg.append('text')
  .attr('class','legend')
  .attr('x', legendX + 10*13)
  .attr('y',legendY + 10)
  .text('More');

d3.csv("assets/data/access_per_day_synt.csv",function(data) {
  ...
  while(l--){
    ...
    var eventDate = data[l].Data.substr(0,10);
    //if the events object doesn't have the current event's day as a key, create a key and give it a value 0
    if(!events[eventDate]){
      events[eventDate] = 0;
    }

    //+ conta to event's value
    events[eventDate] = events[eventDate] + parseInt(data[l].Conta);
  }
  ...
  var range = d3.range(extent[0],extent[1],((extent[1]-extent[0])/9));
